...
const Profile = () => {
    ...
    const postsUrl = `https://cuisineconnoisseursapi.onrender.com/api/posts/profile/${id}`
    const profileUrl = `https://cuisineconnoisseursapi.onrender.com/api/profile/${id}`

    const { data: posts, loading: loading2 } = useFetchData(postsUrl)
    const {
        data: profile,
        setData: setProfile,
        loading,
    } = useFetchData(profileUrl)
    ...
    async function handleSave() {
        try {
            const res = await fetch(
                `https://cuisineconnoisseursapi.onrender.com/api/profile/${id}`,
                {
                    method: 'PATCH',
                    body: JSON.stringify({
                        city: city,
    ...
    async function handleUpload(e) {
        ...
        try {
            const res = await fetch(
                `https://cuisineconnoisseursapi.onrender.com/api/profile/${id}/image`,
                {
                    method: 'POST',
                    body: formData,
                    headers: {
                        ...
                        <div className="col-span-2 flex flex-col gap-2 self-start rounded-xl bg-white drop-shadow-md">
                            <div className="flex flex-col rounded-t-xl bg-slate-900 p-4 text-white min-[850px]:flex-row min-[850px]:items-center">
                                ...
                                <p className="text-xs min-[850px]:ml-auto">
                                    View the posts on the homepage to like and
                                    comment!
                                </p>
                            </div>
